...
# this method needs reviewing -- give sudoku more clues
def solve(c, d, boxes, count):
    if c == 9:
        c = 0
        if d + 1 == 9:
            return count + 1
    # to skip the filled boxes
    if boxes[c][d] != 0:
        return solve(c+1, d, boxes, count)
    # checking to be placed
    for x in range(1, 10):
        if allowed(c, d, x, boxes) and count < 2:
            boxes[c][d] = x
            count = solve(c+1, d, boxes, count)
    boxes[c][d] = 0
    return count
...
while number_of_solutions != 1:
    ...
    for blank_square in random.sample(range(81), 35):
